...
static void getUVPoints(cv::InputArray i_imCurr, std::vector<cv::Point2i> &outvec, int threshVal);

static void getHullPoints(std::vector<cv::Point2i> &uv_points,  HullPoint*& start, double maxConcaveAngle, double similarAngle, cv::InputOutputArray vis_img);

static HullPoint* getSharpestHullPoint(HullPoint* start);

static void getGrid(HullPoint* corner_hull_point, GridPoint*& grid_points, bool& x_axis_first, std::vector<cv::Point2i> &uv_points,  int W, int H, cv::InputOutputArray vis_img);

static int mrWriteMarkers(GridPoint*& grid_points, bool x_axis_first, cv::Size pattern_size, int min_number_of_corners );
  ...
  bool x_axis_first;
	...
	*/
    ...
    /* CV_CALL( storage = cvCreateMemStorage(0) ); */
     thresh_img = cv::Mat( img.rows, img.cols, CV_8UC1 );
     thresh_img_save = cv::Mat( img.rows, img.cols, CV_8UC1 );
    ...
    /* { */
        /* norm_img = cv::Mat( img.rows, img.cols, CV_8UC1 ); */

        /* if( CV_MAT_CN(img.type()) != 1 ) */
        /* { */
            /* CV_CALL( cv::cvtColor( img, norm_img, CV_BGR2GRAY )); */
            /* img = norm_img; */
        /* } */
...
//VISUALIZATION--------------------------------------------------------------
#if VIS
    vis_img = cv::Mat( img.rows, img.cols, CV_8UC1 );
    img.copyTo(vis_img);
    for (auto &pt : uv_points){
      /* std::cout << pt << std::endl; */
      cv::circle (vis_img, pt, 5, cv::Scalar(255));
    }
    cv::namedWindow( "ocv_Markers", 1 );
    cv::imshow( "ocv_Markers", vis_img);
		//cvSaveImage("pictureVis/OrigImg.png", img);
    cv::waitKey(10);
#endif
//END------------------------------------------------------------------------
//
  getHullPoints(uv_points, hull_points, M_PI/4, M_PI/9, vis_img);

  corner_hull_point = getSharpestHullPoint(hull_points);

//VISUALIZATION--------------------------------------------------------------
#if VIS
  cv::circle(vis_img, *(corner_hull_point->point), 15, cv::Scalar(150));
  cv::imshow( "ocv_Markers", vis_img);
  //cvSaveImage("pictureVis/OrigImg.png", img);
  cv::waitKey(10);
#endif
//END------------------------------------------------------------------------
//
  getGrid(corner_hull_point, grid_points, x_axis_first, uv_points, pattern_size.width, pattern_size.height, vis_img);

  found = mrWriteMarkers(grid_points, x_axis_first, pattern_size, pattern_size.width*pattern_size.height);
  error.close();
...
//===========================================================================
static void getUVPoints(cv::InputArray i_imCurr, std::vector<cv::Point2i> &outvec, int threshVal) {
  /* cv::imshow("test",i_imCurr); */
  /* cv::waitKey(0); */
  cv::Mat m_imCurr = i_imCurr.getMat();
      ...
      if (m_imCheck.data[index2d(i, j)] == 0) {
        ...
        if (true) {
          ...
          for (int m = 0; m < (int)(fastPoints.size()); m++) {
            ...
            /* if (((i == 285) && (j == 243)) || (m_imCurr.at<unsigned char>(cv::Point(i, j)) > 100)){ */
            /* if ( (m_imCurr.at<unsigned char>(cv::Point(i, j)) > 5)){ */
            /*     std::cout << "FP " << i << ":" << j << " (" << (int)(m_imCurr.at<unsigned char>(cv::Point(i, j))) << ") from " << x << ":" << y << " (" << (int)(m_imCurr.at<unsigned char>(cv::Point(x, y))) << ") has Diff.) of " << (m_imCurr.at<unsigned char>(cv::Point(i, j)) - m_imCurr.at<unsigned char>(cv::Point(x, y)))<< std::endl; */
            /* } */

            if ((m_imCurr.at<unsigned char>(cv::Point(i, j)) - m_imCurr.at<unsigned char>(cv::Point(x, y))) < (threshVal/2)) {
...
static void getHullPoints(std::vector<cv::Point2i> &uv_points,  HullPoint*& start, double maxConcaveAngle, double similarAngle, cv::InputOutputArray vis_img){
  ...
  //VISUALIZATION--------------------------------------------------------------
#if VIS
  cv::circle(vis_img, uv_points[leftMostCornerIndex], 7, cv::Scalar(255));
  cv::circle(vis_img, uv_points[leftMostCornerIndex], 3, cv::Scalar(255));
  cv::namedWindow( "ocv_Markers", 1 );
  cv::imshow( "ocv_Markers", vis_img);
  cv::waitKey(30);
#endif
//END------------------------------------------------------------------------
  ...
  do {
    ...
    //VISUALIZATION--------------------------------------------------------------
#if VIS
    cv::circle(vis_img, uv_points[bestIndex], 7, cv::Scalar(255));
    cv::namedWindow( "ocv_Markers", 1 );
    cv::imshow( "ocv_Markers", vis_img);
    cv::waitKey(10);
#endif
        ...
        if ((trans_point.x > 0.5) && (trans_point.x < 1.5))
          if ((trans_point.y > 0.5) && (trans_point.y < 1.5)){
            ...
            if ((!found_one) || (found_one && (distance_from_ideal_current<distance_from_ideal_last)) )
            {
              //VISUALIZATION--------------------------------------------------------------
#if VIS
              cv::circle(vis_img, uv_points[i], 12, cv::Scalar(255));
              cv::namedWindow( "ocv_Markers", 1 );
              cv::imshow( "ocv_Markers", vis_img);
              cv::waitKey(10);
#endif
